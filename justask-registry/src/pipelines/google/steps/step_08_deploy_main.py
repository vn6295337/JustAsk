...
"""
Production Deployment Script: Google Data Migration - PostgreSQL Version
==============================================

This script deploys Google data from working_version_v3 to ai_models_main_v3 by:
1. Backing up existing Google records from ai_models_main_v3
2. Deleting existing Google records from ai_models_main_v3
3. Copying Google data from working_version_v3 to ai_models_main_v3
...
# Import database utilities from src.utils
from src.utils import (
    get_record_count,
    backup_records,
    delete_records,
...
# Configuration
LOG_FILE = get_output_file_path("08_deploy_main_report.txt")
...
def restore_production_backup(conn, backup_data: List[Dict[str, Any]]) -> bool:
    ...
    logger.info(f"🔄 Rolling back: Restoring {len(backup_data)} Google records to production...")
    ...
    if insert_records_batch(conn, PRODUCTION_TABLE, clean_backup, batch_size=50):
        ...
        logger.info(f"✅ Rollback successful: {final_count} Google records restored to production")
        return True
    else:
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("GOOGLE PRODUCTION DEPLOYMENT STARTED")
    logger.info("=" * 70)
    ...
    try:
        ...
        if staging_count == 0:
            logger.error("❌ DEPLOYMENT FAILED: No Google data in staging table")
            return False

        # Step 3: Load staging data
        logger.info(f"📁 Loading Google data from {STAGING_TABLE}...")
        ...
        logger.info(f"✅ Loaded {len(staging_data)} Google records from staging")
        ...
        logger.info(f"✅ Backed up {len(backup_data)} existing Google records from production")

        # Step 6: Delete existing production data
        logger.info(f"🗑️ Deleting existing Google records from {PRODUCTION_TABLE}...")
        ...
        logger.info(f"✅ Successfully deleted {production_count} Google records from production")
        ...
        logger.info("=" * 70)
        logger.info("🎉 GOOGLE PRODUCTION DEPLOYMENT COMPLETED SUCCESSFULLY")
        logger.info("=" * 70)
        ...
        logger.info("=" * 70)
        logger.info("🚀 Google models successfully deployed to production!")
