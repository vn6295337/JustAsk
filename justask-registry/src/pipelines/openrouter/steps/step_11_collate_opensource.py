...
Sources:
- 05_hf_license_urls.json (license info URLs)
- 08_opensource_licenses.json (opensource license names)
- 09_opensource_license_urls.json (opensource official URLs)
Output: 11_collated_opensource_licenses.json + comprehensive report
"""
...
def load_special_mappings() -> Dict[str, str]:
    ...
    config_path = CONFIG_DIR / "provider_enrichment.json"
    try:
...
def collate_license_information() -> List[Dict[str, Any]]:
    ...
    # Step 05: License info URLs from HF
    stage_e_models = load_json_file(
        get_input_file_path('05_hf_license_urls.json'),
        'license info URLs'
    )
    
    # Step 08: Opensource license names only
    opensource_names = load_json_file(
        get_input_file_path('08_opensource_licenses.json'),
        'opensource license names'
    )
    
    # Step 09: Opensource URLs
    opensource_urls = load_json_file(
        get_input_file_path('09_opensource_license_urls.json'),
...
def save_collated_data(collated_models: List[Dict[str, Any]]) -> str:
    ...
    output_file = get_output_file_path('11_collated_opensource_licenses.json')
...
def generate_collation_report(collated_models: List[Dict[str, Any]]) -> str:
    ...
    report_file = get_output_file_path('11_collated_opensource_licenses_report.txt')
    ...
    try:
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  Processor           : step_11_collate_opensource.py\n")
            f.write(f"  Output              : 11_collated_opensource_licenses.json\n\n")
            ...
            stage_e_index = create_canonical_slug_index(load_json_file(get_input_file_path('05_hf_license_urls.json'), 'temp'))
            ...
            f.write(f"  Step 05 (License Info URLs)    : {stage_coverage['stage_e']} models\n")
            f.write(f"  Step 08 Opensource Names       : {stage_coverage['stage_h_opensource']} models\n")
            f.write(f"  Step 09 Opensource URLs        : {stage_coverage['stage_i']} models\n\n")
