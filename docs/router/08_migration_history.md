...
Successfully migrated justask-router from using a custom `models` table to using the `working_version` table (output from justask-registry) as the authoritative source for model metadata.
...
**From:** 30 minutes
**To:** 24 hours
**Rationale:** Match database update frequency from justask-registry (daily updates)
...
**Rationale:** Use authoritative source from justask-registry pipeline
...
**Result:** 35/50+ models mapped (approx. 49% coverage)
