...
### Scoring & Complexity Matching

See [09_selection_algorithm.md](09_selection_algorithm.md) for complete details on:
- Multi-factor scoring formula (5 weighted factors)
- Factor definitions and calculations
- Complexity-headroom matching thresholds
- Rate limit headroom calculation (4-metric system)
...
**Key Principles:**
- **working_version:** Read-only, managed by justask-registry pipeline
...
{
  key: 'ai_models_main',
  data: [...],  // 50+ models with 3-table JOIN
  timestamp: Date.now(),
  ttl: 86400000  // 24 hours (matches daily DB updates)
}
```
...
### JustAsk API Integration

**File:** `justask/api/src/utils/modelSelectorClient.js`
