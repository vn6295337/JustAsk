# JustAsk Router - Developer Guide for Claude
...
**justask-router** is a microservice that dynamically selects optimal AI models from the ai_models_main Supabase table based on query characteristics, performance metrics, rate limits, and geographic considerations.

**Core Purpose:** Replace hardcoded model selection in JustAsk API with intelligent, data-driven routing.
...
**Complexity Scoring:**
- ⚠️ Must be calculated by client (JustAsk API)
...
## Integration with JustAsk API

### Modified Files

**justask/api/src/routing/router.js:**
```javascript
// Before
...
**justask/api/src/failover/failover.js:**
```javascript
// Before
...
**justask/api/src/utils/modelSelectorClient.js:**
```javascript
...
**Related Projects:**
- `justask-registry/` - Populates ai_models_main table
- `justask/api/` - Consumer of selector service
